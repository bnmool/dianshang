...
const getPathMap = (skus) => {
  ...
  skus.forEach((sku) => {
    if (sku.inventory > 0) {
      ...
      // 可选规格数组
      const valueArr = sku.specs.map((val) => {
...
const getSelectedValues = (specs) => {
  ...
  specs.forEach((item) => {
    // 选中的按钮对象
    const seletedVal = item.values.find((val) => {
      return val.selected;
    });
...
const initDefaultSelected = (goods, skuId) => {
  ...
  goods.specs.forEach((item, i) => {
    const val = item.values.find((val) => {
      return val.name === sku.specs[i].valueName;
    });
    val.selected = true;
  });
};
export default {
  ...
  setup(props, { emit }) {
    ...
    const changeSku = (item, val) => {
      ...
      // ☆点击按钮时：更新按钮禁用状态
      updateDisabledStatus(props.goods.specs, pathMap);
      // 触发change事件将sku数据传递出去 {skuId,price,oldPrice,inventory,specsText}
